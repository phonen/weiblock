...
class UsersModel extends BaseModel {
	...
	public function register($userinfo){
		...
		if($rs['status']==1){
			...
			$data['redPacketMoney'] = $GLOBALS['CONFIG']['first_login_money'];
	...
	public function addDistributMoney($obj){
	   ...
	   if ($first['userId']>0){
	        M('Users')->where(array('userId'=>$first['userId']))->setInc('userMoney',$firt_commission);
			M('Users')->where(array('userId'=>$first['userId']))->setInc('distributMoney',$firt_commission);
	        //写记录
	        $moneyRemark = '获得一级下线下注佣金';
	        $userMoney = $first['userMoney']+$firt_commission;
			$distributMoney = $first['distributMoney']+$firt_commission;
	        addMoneyLog($first['userId'], $moneyRemark, '1', $dataId, $firt_commission, $transactionId, 2, 1,$userMoney,$first['redPacketMoney'],$distributMoney);
	   ...
	   if ($second['userId']>0){
	        M('Users')->where(array('userId'=>$second['userId']))->setInc('userMoney',$second_commission);
			M('Users')->where(array('userId'=>$second['userId']))->setInc('distributMoney',$second_commission);
	        //写记录
	        $moneyRemark = '获得二级下线下注佣金';
	        $userMoney = $second['userMoney']+$second_commission;
			$distributMoney = $second['distributMoney']+$second_commission;
	        addMoneyLog($second['userId'], $moneyRemark, '1', $dataId, $second_commission, $transactionId, 2, 1,$userMoney,$second['redPacketMoney'],$distributMoney);
	   ...
	   if ($three['userId']>0){
	        M('Users')->where(array('userId'=>$three['userId']))->setInc('userMoney',$three_commission);
			M('Users')->where(array('userId'=>$three['userId']))->setInc('distributMoney',$three_commission);
	        //写记录 
	        $moneyRemark = '获得三级下线下注佣金';
	        $userMoney = $three['userMoney']+$three_commission;
			$distributMoney = $three['distributMoney']+$three_commission;
	        addMoneyLog($three['userId'], $moneyRemark, '1', $dataId, $three_commission, $transactionId, 2, 1,$userMoney,$three['redPacketMoney'],$distributMoney);
	   ...
	   if ($four['userId']>0){
	        M('Users')->where(array('userId'=>$four['userId']))->setInc('userMoney',$four_commission);
			M('Users')->where(array('userId'=>$four['userId']))->setInc('distributMoney',$four_commission);
	        //写记录 
	        $moneyRemark = '获得四级下线下注佣金';
	        $userMoney = $four['userMoney']+$four_commission;
			$distributMoney = $four['distributMoney']+$four_commission;
	        addMoneyLog($four['userId'], $moneyRemark, '1', $dataId, $four_commission, $transactionId, 2, 1,$userMoney,$four['redPacketMoney'],$distributMoney);
	   ...
	   if ($five['userId']>0){
	        M('Users')->where(array('userId'=>$five['userId']))->setInc('userMoney',$five_commission);
			M('Users')->where(array('userId'=>$five['userId']))->setInc('distributMoney',$five_commission);
	        //写记录 
	        $moneyRemark = '获得五级下线下注佣金';
			$userMoney = $five['userMoney']+$five_commission;
			$distributMoney = $five['distributMoney']+$five_commission;
	        addMoneyLog($five['userId'], $moneyRemark, '1', $dataId, $five_commission, $transactionId, 2, 1,$userMoney,$five['redPacketMoney'],$distributMoney);
