...
class CashDrawsModel extends BaseModel {
	...
	public function condition(){
		$condition = array();
		$condition['user_id'] = (int)session('WST_USER.userId');
		$condition['recharge_status'] = '1';
	    $recharge_money = M('recharge')->where($conditon)->sum('recharge_money');
	    
	    $condition2 = array();
	    $condition2['user_id'] = (int)session('WST_USER.userId');
	    $condition2['is_pay'] = '1';
	    $trade_money = M('trade')->where($condition2)->sum('trade_money');
	    
	    $check = $trade_money%$recharge_money;
	    if($check < 0.3){
