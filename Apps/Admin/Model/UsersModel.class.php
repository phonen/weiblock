...
class UsersModel extends BaseModel {
    /**
     ...
     public function queryByPage(){
        $map = array();
	 	...
	 	if(I('userId')!='')$sql.=" and userId = ".I('userId')." ";
	 ...
	 public function getAccountById(){
		 $rs = $this->where('userId='.(int)I('id',0))->getField('userId,loginName,userStatus,userType,userMoney,redPacketMoney,distributMoney',1);
		 return current($rs);
	 }
	 /**
	  * 修改账号信息
    ...
    /**
	 ...
	 public function recharge(){
	 	$rd = array('status'=>-1);
	 	if(I('id')=='')return $rd;
	 	if (I('money')<1)return $rd;
	 
	 	$data1 = array ();
		$data1 ["user_id"] = I('id'); //用户id
		...
		$data1 ["recharge_status"] = 1; 
	 	...
	 	$userInfo = M('users')->where('userId='.(int)I('id'))->field('userId,loginName,userStatus,userType,userMoney,redPacketMoney,distributMoney')->find();
	    if(false !== $rs){
	    	...
	    	$distributMoney = $userInfo['distributMoney'];
			   ...
			   $data["moneyRemark"] = '通过后台成功充值:'.I('money');
			   $data["dataSrc"] = 2;//充值订单
			   $data["dataId"] =  $rs;
			   $data["money"] = I ('money');
	    	...
	    	$this->where("userId=".(int)I('id'))->setInc('userMoney',I('money'));
			$rd['status']= 1;
		}
		return $rd;
	 }
	 ...
	 public function decMoney(){
	    ...
	    if(false !== $rs){
	    	...
	    	$distributMoney = $userInfo['distributMoney'];
			   ...
			   $data["targetType"] = 0;
			   $data["targetId"] = $userInfo['userId'];
			   $data["moneyRemark"] = '通过后台成功扣除:'.I('money');
			   $data["dataSrc"] = 2;//充值订单
			   $data["dataId"] =  $rs;
			   $data["money"] = I ('money');
			   $data["transactionId"] = $data1 ["recharge_sn"];
			   ...
			   $data["distributMoney"] = $userInfo['distributMoney'];
			   M('log_moneys')->add($data);
	    	$this->where("userId=".(int)I('id'))->setDec('userMoney', I('money'));
			$rd['status']= 1;
		}
		return $rd;
	 }
