...
class CashDrawsModel extends BaseModel {
    ...
    public function queryByPage($type){
		...
		$sql = "select c.*,u.loginName,u.userName from __PREFIX__cash_draws c left join __PREFIX__users u on c.targetType=0 and targetId=u.userId 
		        where 1=1 ";
		if(!empty($userID))
		{
			$sql.="and c.targetId=".$userID;
		}
        if($targetType!=-1){
	...
	public function get(){
		$id = (int)I('id');
		$sql = "select* from __PREFIX__cash_draws c left join  u on targetId=u.userId where 1=1 ";
