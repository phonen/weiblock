...
class CashDrawsAction extends BaseAction{
   /**
	...
	function pay(){
		$this->pay_run(I('drawMoney'),I('targetType'));
	}
     ...
     */
    function pay_run($money,$targetType){
    	$this->isLogin();
    	if (IS_POST){
	    	$rd = array('status'=>-1);
			...
			$addData = array();
	       	...
	       	$addData['money']=$money;
	       	$addData['change_money']=$money*$draw_rate;
	       	$addData['cashSatus']='0';
	        ...
	        if ($insert_id>0){
	        	...
	        	{
	        		if ($targetType==0){
	        			M('users')->where(array('userId'=>$userId))->setInc('lockMoney',$money*(1+$draw_rate));
	        			M('users')->where(array('userId'=>$userId))->setDec('userMoney',$money*(1+$draw_rate));
	        		}else{
	        			M('users')->where(array('userId'=>$userId))->setInc('lockMoney',$money*(1+$draw_rate));
	        			M('users')->where(array('userId'=>$userId))->setDec('distributMoney',$money*(1+$draw_rate));
	        		}
	        		//提现需要审核
	        		$rd = array('code'=>'200','status'=>'1','msg'=>'提交成功,请等待管理员审核~~');
	        	}else{
					//提现无需审核，处理提现
	...
	public function handle($draw_sn){
	   ...
	   if($rs['targetType']==1){
		...
		if($rs['cashSatus']==0){
			...
			$post_data = array (
	            'mid' => '624', //在掌上零钱里面获取的uid
	            'jine' => $rs['money'], //要请求发放的金额
	            'openid'=>session('openid1'), //第二步获取的openid
	            'tixianid'=>time(), //本地的提现id【要求唯一】字符串类型的数字，最大长度11位数
	            'lailu' =>'sd', //可选参数
			);
			...
			if ($data['o'] == 'yes')
			{
				...
				$this->ajaxReturn(array('code'=>'200','status'=>'1','msg'=>'提现成功!'));
			}else{
