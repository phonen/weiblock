    ...
    */
	...
	/**
	 ...
	 public function addByVisitor(){
		...
		if(false !== $userId){
			...
			$s->isDistributAll = (int)I("isDistributAll",0);
			$s->shopFlag = 1;
	...
	public function getDistrictsShops($obj){
		...
		for($k=0;$k<count($ctlist);$k++){
			...
			$sql = "SELECT * from ( 
						select sp.shopId,shopName,deliveryStartMoney,deliveryMoney,shopAtive from __PREFIX__shops_communitys sc,__PREFIX__shops sp WHERE sp.shopStatus = 1 AND sp.shopFlag = 1 AND sc.shopId = sp.shopId AND communityId=".$communityId."
						UNION ALL
						select shopId,shopName,deliveryStartMoney,deliveryMoney,shopAtive from __PREFIX__shops sp WHERE shopStatus = 1 AND shopFlag = 1 AND isDistributAll=1 
					) shops where 1=1 ";
	...
	public function getShopByCommunitys($obj){
		...
		$dsplist = array();
		$sql = "SELECT * from (
					select sp.shopId,shopName,shopAddress,deliveryStartMoney,shopAtive,deliveryMoney,shopImg,deliveryCostTime,deliveryFreeMoney,avgeCostMoney from __PREFIX__shops_communitys sc,__PREFIX__shops sp WHERE sp.shopStatus = 1 AND sp.shopFlag = 1 AND sc.shopId = sp.shopId AND communityId=".$communityId."
					UNION ALL
					select shopId,shopName,shopAddress,deliveryStartMoney,shopAtive,deliveryMoney,shopImg,deliveryCostTime,deliveryFreeMoney,avgeCostMoney from __PREFIX__shops WHERE shopStatus = 1 AND shopFlag = 1 AND isDistributAll=1 
				) shops WHERE 1 = 1 ";
