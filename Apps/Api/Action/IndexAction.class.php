...
class IndexAction extends BaseAction{
  ...
  public function hnvpayNotify(){
	
    if($_POST){
		
        $post=$_POST;
        $data["textcont"]=json_encode($post);
        $sign=md5($_POST["order_id"].$_POST["orderNo"].$_POST["money"].$_POST["mch"].$_POST["pay_type"].$_POST["time"].md5("07d1e9b135b6737bc39b11a7e16aef72"));
        if($sign==$_POST['sign']){
            $order_id=trim($_POST['order_id']);
			$pay_no=$_POST["orderNo"];
			$money=$_POST["money"]/100;
			
  			$recharge = M('recharge')->where(array('recharge_sn'=>$order_id))->find();
			
  			
  			if (!$recharge){
  				exit('ERROR'); //返回失败 支付信息不存在
  			}
			...
			$data1 = array ();
  			...
  			$rs = M('recharge')->where(array('recharge_sn'=>$order_id))->save($data1);

  			if($rs){
				$userInfo = M('users')->where(array('userId'=>$recharge['user_id']))->field('userId,loginName,userStatus,userType,userMoney,redPacketMoney,distributMoney')->find();
  				...
  				$data["dataSrc"] = 2;//充值订单
  				$data["dataId"] =  $recharge['recharge_id'];
  				$data["money"] = $money;
  				$data["transactionId"] = $recharge["recharge_sn"];
  				$data["type"] = 0;
  				$data["moneyType"] = 1;
